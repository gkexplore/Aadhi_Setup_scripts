		...
		def start_aadhi
			...
			system_or_exit("echo #{$pwd} | sudo -S chmod -R 777 /var/www/scenario_server_mysql/tmp")
			system_or_exit("open http://localhost")		
		end
		...
		def give_permissions
			...
			Dir.chdir("/var/www/scenario_server_mysql")
			system_or_exit("echo #{$pwd} | sudo -S chmod -R 777 /var/www/scenario_server_mysql")
			system_or_exit("echo #{$pwd} | sudo -S chmod -R 777 /var/www/scenario_server_mysql/public/assets")
		...
		def install_gems_and_mysql
			...
			system_or_exit("echo #{$pwd} | sudo -S chmod -R 777 /var/www/scenario_server_mysql/db")
...
=begin
		...
		def clone_scenario_server(password, environment, path)
			...
			system_command("echo #{password} | sudo -S chmod -R 777 /var/www/scenario_server_mysql/db", "Giving write access to Aadhi server DB related files (chmod -R 777 /var/www/scenario_server_mysql/db)")
			case environment
			when "d"
		...
		def give_permissions(password)
			...
			Dir.chdir("/var/www/scenario_server_mysql")
			system_command("echo #{password} | sudo -S chmod -R 777 /var/www/scenario_server_mysql", "chmod -R 777 /var/www/scenario_server_mysql")
			system_command("echo #{password} | sudo -S chmod -R 777 /var/www/scenario_server_mysql/public/assets", "chmod -R 777 /var/www/scenario_server_mysql/public/assets")
			...
			system_command("echo #{password} | sudo -S chmod -R 777 /var/www/scenario_server_mysql/tmp", "Giving permission to tmp folder (chmod -R 777 /var/www/scenario_server_mysql/tmp)")
