    ...
    then
        if [[ $RUBY_VERSION == *"2.2.0"* ]]
          then
           ...
           #rbenv global 2.2.0            
           rbenv-gemset create 2.2.0 aadhi
        else
           rbenv install 2.2.0
           #rbenv global 2.2.0            
           rbenv-gemset create 2.2.0 aadhi
...
function clone_and_install_aadhi
{
  ...
  if [ -d "/var/www/scenario_server_mysql" ]; then
    echo "$PASSWORD" | sudo -S rm -rf /var/www
    cd /var/
    echo "$PASSWORD" | sudo -S mkdir www
  elif [ ! -d "/var/www/scenario_server_mysql" ]; then
    cd /var/
    echo "$PASSWORD" | sudo -S mkdir www
  fi
  cd $CURRENT_PATH
  ...
  echo "$PASSWORD" | sudo -S echo '2.2.0' >> .ruby-version
...
function install_and_setup_mysql
{
  ...
  echo "$PASSWORD" | sudo -S chmod -R 777 /var/www/scenario_server_mysql/db
...
function give_permission_to_aadhi_folders
{
  ...
  echo "$PASSWORD" | sudo -S chmod -R 777 /var/www/scenario_server_mysql
  echo "$PASSWORD" | sudo -S chmod -R 777 /var/www/scenario_server_mysql/public/assets
}
...
function configure_apache
{
  ...
  open http://localhost
  sleep 5
  echo "$PASSWORD" | sudo -S chmod -R 777 /var/www/scenario_server_mysql/tmp
  sudo apachectl -k restart
  open http://localhost
}
